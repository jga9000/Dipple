...
void Box2DBody::stop()
{
    /*if (Box2DWorld *world = qobject_cast<Box2DWorld*>(parent()))
    {
        ...
        //emit unregisterBody();
        destroyed();
        qDebug() << "Unregistered body";
    }*/
    destroyed();
}

void Box2DBody::activate()
{
    if (Box2DWorld *world = qobject_cast<Box2DWorld*>(parent())) {
        qDebug() << "activating body";
        world->appendBody(this);
    }
    else {
        qDebug() << "Warning: activated Body should have World as parent";
    }
}

void Box2DBody::move(int x, int y, int x2, int y2)
{
    qDebug() << "Box2DBody::move called";
    qDebug() << "x:" << x << "y:" << y << "x2:" << x2 << "y2:" << y2;
    float impact = 0.25;    // This is the variable for the impact energy   (0.25 bit weak, 0.35 moderate, 0.50 way too high)
    float32 xVec = float(x2-x);
    xVec = xVec*impact;
    if( xVec > 20 ) xVec = 20;
    float yVec = float(y2-y);
    yVec = yVec*impact;
    if( yVec > 2 ) yVec = 2;
    qDebug() << "xVec:" << xVec << "yVec:" << yVec;
    b2Vec2 force = b2Vec2(xVec, yVec);
    force.Normalize();
    mBody->ApplyLinearImpulse(force, mBody->GetPosition());
}

void Box2DBody::changePosition(int xPos, int yPos)
{
    /*
    Q_ASSERT(mBody);
    //const qreal newX = (qreal)xPos/100;// * scaleRatio - width() / 2;
    //const qreal newY = (qreal)yPos/100;// * scaleRatio - height() / 2;

    const qreal newX = (qreal)xPos / (2 * 32.0f);// * scaleRatio - width() / 2;
    const qreal newY = (qreal)-yPos / (2 * 32.0f);// * scaleRatio - height() / 2;

    //const qreal newX = (x() + width() / 2) / scaleRatio;
    //const qreal newY = -(y() + height() / 2) / scaleRatio;

    b2Vec2 position;
    position.Set(newX, newY);

    qDebug() << "Box2DBody::changePosition called";
    ...
    */
    qDebug() << "Box2DBody::changePosition called";
    mBody->SetTransform(b2Vec2(xPos / scaleRatio,
                               -yPos / scaleRatio),
                        mBody->GetAngle() );
